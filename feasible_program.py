...
meal_plan = LpProblem("DietOptimization", LpMinimize)
...
meal_plan += lpSum([x[i][r] * cost[i][r] for i in food_items for r in restaurants]), "Total Cost"

# Define the nutritional requirements as constraints
meal_plan += lpSum([x[i][r] * calories[i][r] for i in food_items for r in restaurants]) >= 2350, "Calories"
meal_plan += lpSum([x[i][r] * protein[i][r] for i in food_items for r in restaurants]) >= 7, "Protein"
meal_plan += lpSum([x[i][r] * vitaminA[i][r] for i in food_items for r in restaurants]) >= 725, "VitaminA"
meal_plan += lpSum([x[i][r] * vitaminC[i][r] for i in food_items for r in restaurants]) >= 100, "VitaminC"
meal_plan += lpSum([x[i][r] * vitaminB1[i][r] for i in food_items for r in restaurants]) >= 1.15, "VitaminB1"
meal_plan += lpSum([x[i][r] * vitaminB2[i][r] for i in food_items for r in restaurants]) >= 1.35, "VitaminB2"
meal_plan += lpSum([x[i][r] * niacin[i][r] for i in food_items for r in restaurants]) >= 15, "Niacin"
meal_plan += lpSum([x[i][r] * calcium[i][r] for i in food_items for r in restaurants]) >= 750, "Calcium"
meal_plan += lpSum([x[i][r] * phosphorus[i][r] for i in food_items for r in restaurants]) >= 700, "Phosphorus"
meal_plan += lpSum([x[i][r] * natrium[i][r] for i in food_items for r in restaurants]) <= 2000, "Natrium"
meal_plan += lpSum([x[i][r] * potassium[i][r] for i in food_items for r in restaurants]) >= 3500, "Potassium"
meal_plan += lpSum([x[i][r] * iron[i][r] for i in food_items for r in restaurants]) >= 12, "Iron"

# Define the budget constraint
meal_plan += lpSum([x[i][r] * cost[i][r] for i in food_items for r in restaurants]) <= Budget_max, "Budget"
